...
ClapTrap::ClapTrap(std::string name, int hitPoints, int energyPoints, int attackDamage):dead(false) {
...
void ClapTrap::attack(const std::string &target) {
    if (getEnergyPoints() <= 0 && !isDead()) {
        std::cout << "Cant attack " << target << ". ClapTrap " << getName() << " is out of energy" << std::endl;
        return;
    } else if (isDead()) {
        std::cout << "Cant attack " << target << ". ClapTrap " << getName() << " is dead" << std::endl;
        return;
    }
...
void ClapTrap::takeDamage(unsigned int amount) {
    if (isDead()) {
        std::cout << "ClapTrap " << getName() << " cant take damage. He is already dead" << std::endl;
        return;
    }
...
void ClapTrap::beRepaired(unsigned int amount) {
    if (getEnergyPoints() <= 0 && !isDead()) {
        std::cout << "ClapTrap " << getName() << " cant repair itself, because it is out of energy" << std::endl;
        return;
    } else if (isDead()) {
        std::cout << "ClapTrap " << getName() << " cant repair itself. he is dead" << std::endl;
        return;
    }
    std::cout << "ClapTrap " << getName() << " repaired itself " << amount << " times" << std::endl;
    setHitPoints(amount);
    setEnergyPoints(getEnergyPoints() - 1);
}
